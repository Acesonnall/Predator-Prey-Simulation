...
public abstract class Living {
	...
	protected void census(int population[]) {
		int GRID_START = 0;
		int rowStart = Math.max(row - 1, GRID_START);
		int rowEnd = Math.min(row + 1, world.grid.length - 1);
		int colStart = Math.max(column - 1, GRID_START);
		int colFinish = Math.min(column + 1, world.grid.length - 1);

		for (int curRow = rowStart; curRow <= rowEnd; curRow++) {
			for (int curCol = colStart; curCol <= colFinish; curCol++) {
				if (world.grid[curRow][curCol].who() == State.BADGER) {
					population[BADGER]++;
				} else if (world.grid[curRow][curCol].who() == State.EMPTY) {
					population[EMPTY]++;
				} else if (world.grid[curRow][curCol].who() == State.FOX) {
					population[FOX]++;
				} else if (world.grid[curRow][curCol].who() == State.GRASS) {
					population[GRASS]++;
				} else if (world.grid[curRow][curCol].who() == State.RABBIT) {
					population[RABBIT]++;
				}
			}
		}
